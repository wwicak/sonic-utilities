...
class Utils:
    ...
    def get_config_db_as_json(self, namespace):
        ...
        if namespace is not None and namespace != multi_asic.DEFAULT_NAMESPACE:
            cmd += ['-n', namespace]
        stdout, return_code = self.get_cmd_output(cmd)
        if return_code:
...
class ConfigWrapper:
    ...
    def get_sonic_yang_as_json(self):
        config_db_json = utils.get_config_db_as_json(self.namespace)
...
class DryRunConfigWrapper(ConfigWrapper):
    ...
    def _init_imitated_config_db_if_none(self):
        ...
        if self.imitated_config_db is None:
            self.imitated_config_db = super().get_config_db_as_json(self.namespace)


class PatchWrapper:
    def __init__(self, namespace, config_wrapper=None):
        self.namespace = namespace
        self.config_wrapper = config_wrapper if config_wrapper is not None else ConfigWrapper(namespace)
    ...
    def convert_config_db_patch_to_sonic_yang_patch(self, patch):
        ...
        current_config_db = utils.get_config_db_as_json(self.namespace)

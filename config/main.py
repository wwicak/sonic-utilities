...
@click.option('-f', '--format', type=click.Choice([e.name for e in ConfigFormat]),
               ...
               show_default=True)
@click.option('-p', '--parallel', is_flag=True, default=False, help='applying the change parallelly')
...
@click.pass_context
def apply_patch(ctx, patch_file_path, format, parallel, dry_run, ignore_non_yang_tables, ignore_path, verbose):
    ...
    try:
        ...
        if multi_asic.is_multi_asic():
            ...
            if parallel:
                ...
                for asic_id, patch in changes_by_asic.items():
                    GenericUpdater(asic_id).apply_patch(patch, config_format, verbose, dry_run, ignore_non_yang_tables, ignore_path)
        else:
...
def apply_patch_wrapper(args):
    # Unpack arguments
    asic_id, changes, config_format, verbose, dry_run, ignore_non_yang_tables, ignore_path = args
    GenericUpdater(asic_id).apply_patch(changes, config_format, verbose, dry_run, ignore_non_yang_tables, ignore_path)
